...
def main():
    ...
    # Get POC05_v02 table
    poc05_v02_table = config['rag_config']['poc05_v02_table']
    print(f"\n[2] Loading: {poc05_v02_table}")

    df = spark.table(poc05_v02_table)
    row_count = df.count()
    ...
    # Save to new table
    poc05_v02_table = f"{config['rag_config']['full_namespace']}.poc05_v02_chunks_with_metadata"

    print(f"\n[5] Saving to: {poc05_v02_table}")
    df_with_metadata.write.format("delta").mode("overwrite").saveAsTable(poc05_v02_table)
    print(f"    Saved {row_count} chunks")

    # Verify
    print(f"\n[6] Verifying...")
    df_verify = spark.table(poc05_v02_table)
    ...
    config['rag_config']['poc05_v02_metadata_table'] = poc05_v02_table
    config['rag_config']['poc05_v02_metadata_count'] = row_count
    config['rag_config']['poc05_v02_metadata_created_at'] = __import__('time').strftime("%Y-%m-%dT%H:%M:%S")
    ...
    print(f"\nNext: Create POC05_v02 vector index with these metadata columns for filtering")
