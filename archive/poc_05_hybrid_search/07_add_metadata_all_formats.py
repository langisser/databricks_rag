#!/usr/bin/env python3
"""
POC05_V02 All Formats - Add Metadata to 28,823 chunks
Source: poc05_v02_all_formats_chunks (docx, pdf, xlsx)
...
def main():
    print("=" * 80)
    print("POC05_V02 - ADD METADATA USING SQL")
    print("=" * 80)
    ...
    # Get all formats table
    source_table = config['rag_config']['poc05_v02_all_formats_table']
    ...
    # Save to new table
    output_table = f"{config['rag_config']['full_namespace']}.poc05_v02_all_formats_with_metadata"
    ...
    config['rag_config']['poc05_v02_all_formats_metadata_table'] = output_table
    config['rag_config']['poc05_v02_all_formats_metadata_count'] = row_count
    config['rag_config']['poc05_v02_all_formats_metadata_created_at'] = __import__('time').strftime("%Y-%m-%dT%H:%M:%S")
    ...
    # Summary
    print("\n" + "=" * 80)
    print("METADATA ADDITION COMPLETE - ALL FORMATS")
    print("=" * 80)
    print(f"\nTable: {output_table}")
    print(f"Total: {row_count} chunks (docx + pdf + xlsx)")
    ...
    print(f"\nNext: Create vector index for all formats with hybrid search")
