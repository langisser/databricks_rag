...
IMPROVEMENTS:
1. RecursiveCharacterTextSplitter from LangChain (respects semantic boundaries)
2. PPTX support with python-pptx (slide-based chunking)
3. Increased overlap from 200 to 300 chars (30% overlap)
4. Better semantic coherence with priority separators

COMPARISON TO POC05:
- POC05: Fixed 800-char sliding window, breaks mid-sentence
- POC06: Semantic chunking, respects paragraphs/sentences, adds PPTX

Output: poc06_phase1_all_formats_chunks
"""
...
# ============================================================================
# PHASE 1 IMPROVEMENT: RecursiveCharacterTextSplitter
# ============================================================================
def chunk_text_semantic(text, chunk_size=1000, chunk_overlap=300):
    """
    Semantic text chunking using LangChain's RecursiveCharacterTextSplitter

    IMPROVEMENTS over POC05:
    - Respects paragraph boundaries (\n\n)
    - Falls back to sentence boundaries (. )
    - Falls back to word boundaries ( )
    - Increased chunk size: 800 -> 1000
    - Increased overlap: 200 -> 300 (30%)
    """
...
def main():
    print("=" * 80)
    print("POC06 PHASE 1 - IMPROVED CHUNKING IMPLEMENTATION")
    print("=" * 80)
    print("\nIMPROVEMENTS:")
    print("1. [STATUS] RecursiveCharacterTextSplitter (semantic boundaries)")
    print("2. [STATUS] PPTX support with slide-based chunking")
    print("3. [STATUS] Increased overlap: 200 -> 300 chars (30%)")
    print("4. [STATUS] Chunk size: 800 -> 1000 chars")
        ...
        try:
            ...
            if chunks and file_type:
                for i, chunk in enumerate(chunks):
                    chunk_id = f"poc06_phase1_{processed_count}_{i}"
                    chunk_row = {
                        'id': chunk_id,
    ...
    output_table = "sandbox.rdt_knowledge.poc06_phase1_all_formats_chunks"
    ...
    # Update config
    config['rag_config']['poc06_phase1_table'] = output_table
    config['rag_config']['poc06_phase1_count'] = len(all_chunks)
    config['rag_config']['poc06_phase1_stats'] = stats
    config['rag_config']['poc06_phase1_created_at'] = datetime.now().isoformat()

    with open(config_path, 'w') as f:
        json.dump(config, f, indent=2)

    print("\n" + "=" * 80)
    print("POC06 PHASE 1 COMPLETE")
    print("=" * 80)
    print(f"\nTable: {output_table}")
    ...
    print(f"  PPTX: {stats['pptx']['chunks']} chunks from {stats['pptx']['success']} files [NEW!]")
    print(f"\nIMPROVEMENTS APPLIED:")
    print(f"  [STATUS] Semantic chunking (respects boundaries)")
    print(f"  [STATUS] PPTX support (+35 files, +19% coverage)")
    print(f"  [STATUS] 30% overlap (was 25%)")
    print(f"  [STATUS] 1000 char chunks (was 800)")
    print(f"\nNext: Add metadata and create vector index")
