#!/usr/bin/env python3
"""
POC06_v02 - Add Metadata to Improved Chunks
Source: poc06_v02_all_formats_chunks (docx, pdf, xlsx, pptx)
...
def main():
    print("=" * 80)
    print("POC06_v02 - ADD METADATA (ALL FORMATS + PPTX)")
    print("=" * 80)
    ...
    # Get source table
    source_table = config['rag_config']['poc06_v02_table']
    ...
    # Save to new table
    output_table = f"{config['rag_config']['full_namespace']}.poc06_v02_with_metadata"
    ...
    config['rag_config']['poc06_v02_metadata_table'] = output_table
    config['rag_config']['poc06_v02_metadata_count'] = row_count
    config['rag_config']['poc06_v02_metadata_created_at'] = __import__('time').strftime("%Y-%m-%dT%H:%M:%S")
    ...
    # Summary
    print("\n" + "=" * 80)
    print("POC06_v02 - METADATA COMPLETE")
    print("=" * 80)
    print(f"\nTable: {output_table}")
